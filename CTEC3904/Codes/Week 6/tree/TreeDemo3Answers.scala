...
/*
 * Some of this code (split, join, build, shows) is adapted from code originally written by
 * Mark Jones c. (early-1990s) and provided as code examples within the distributions of the
 * FP lanaguages gofer and hugs - both of which had Haskell-like syntax. Haskell went on to
 * become the dominant non-strict, purely functional programming language. However, the early
 * versions of gofer (and subsequently hugs) provided excellent interactive environments for
 * using and learning about early Haskell-style syntax and behaviour.
 *
 * The adaptation here uses the object syntax of Scala for many of the methods.
 * Scala translation and rest of code: DRS
 *
 * This example shows how a tree data structure can be defined. It uses the method syntax
 * predominantly. For this reason, the tree manipulation functions are spread across the
 * classes so that each type of object has its own (overridden) behaviour. Therefore:
 *
 * BST     - an abstract class containing some default methods and method signatures
 *   Empty - the (concrete) class representing empty BSTs. The methods overridden in the
 *           Empty class are written for a receiver (this) which is an Empty tree.
 *   Fork  - the (concrete) class representing non-empty BSTs. A non-empty tree is
 *           constructed from two other (sub)trees (either or both of which may be Empty).
 *           The two sub-trees are named l (left) and r (right) and the node itself
 *           stores data. The methods overridden in the Fork class are written for a
 *           receiver (this) that has a value (n) and two subtrees, l and r.
 */

object TreeDemo3Answers {
  ...
  sealed abstract class BST[+A] {
    ...
    def depth: Int =
      if (this.isEmpty)
        ...
        1 + (this.left.depth max this.right.depth)
    ...
    def contains[B >: A](x: B)(implicit ord: Ordering[B]): Boolean =
      if (this.isEmpty)
        false
      else if (ord.lt(x, this.root))
        this.left.contains(x)
      else if (ord.gt(x, this.root))
    ...
    def preorder: List[A] =
      if (this.isEmpty)
        ...
        this.root :: this.left.preorder ::: this.right.preorder
  }
