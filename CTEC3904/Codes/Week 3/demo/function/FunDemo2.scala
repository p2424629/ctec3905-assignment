...
object FunDemo2 {
  ...
  */
  val roots: Double => Double => Double => (Double, Double) = itShould("return the roots of a quadratic equation")
  // Use the formula: roots(a)(b)(c) = (-b + sqrt(b*b - 4*a*c)/(2*a), b - sqrt(b*b - 4*a*c)/(2*a))
  // Use local definitions to avoid repeating calculations

  def clock(seconds: Int): (Int, Int, Int) = itShould("convert seconds into hours, minutes, and seconds")
