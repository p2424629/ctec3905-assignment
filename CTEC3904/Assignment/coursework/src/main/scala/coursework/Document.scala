...
object Document {
  ...
  class Section(
    ...
    override def toString: String = {
      val indentation = "    "
      val subs = subsections map (_.toString) flatMap
        ((_: String).split("\\n") map (indentation + _))
     ...
     */
    def performSectionNumbering(ns: List[Int]): Section = {
     ...
     */
    def deleteSubsection(number: List[Int]): Section =
      number match {
        case List(m: Int) if m > 0 && m <= subsections.length =>
          subsections.splitAt(m - 1) match {
            case (left, s :: right) =>
     ...
     */
    def getSubsection(number: List[Int]): Section =
      number match {
  ...
  class Book(title: String, chapters: List[Section] = List.empty) {
     ...
     * |  Section E
     *
     * Then b.insertSubZero("1,3", s) would generate:
     *
     * |Chapter X
     * |  s
     * |  Section A
  ...
  def main(args: Array[String]): Unit = {
    ...
    //         println(compsci.lift(2, _.insertSubsection("1.5", testSection)))
    //
