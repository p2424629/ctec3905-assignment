...
object FunDemo2Answers {
   ...
   * argument by partially applying mul.
   */
  ...
  def flipSub(x: Int, y: Int): Int = y - x
  ...
  */
  val roots: Double => Double => Double => (Double, Double) = a => b => c => {
    val s = Math.sqrt(b * b - 4 * a * c)
    val d = 2 * a
    ((-b + s) / d, (b + s) / d)
  }
